...
export default function PerUpdateForm(props) {
  const {
    id: idProp,
    per: perModelProp,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = perRecord
      ? { ...initialValues, ...perRecord }
      : initialValues;
  ...
  const [perRecord, setPerRecord] = React.useState(perModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp ? await DataStore.query(Per, idProp) : perModelProp;
      setPerRecord(record);
    };
    queryData();
  }, [idProp, perModelProp]);
  React.useEffect(resetStateValues, [perRecord]);
  const validations = {
    name: [{ type: "Required" }],
    age: [],
    breed: [],
  ...
  return (
    <Grid
      ...
      {...getOverrideProps(overrides, "PerUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || perModelProp)}
